    ...
    def __init__(
        ...
        if hostname:
            try:
                ...
                self.service = gssapi.Name(f"HTTP/{hostname}@").canonicalize(
                    gssapi.MechType.kerberos
                )
            except gssapi.GSSError as exc:
                self.logger.warning(
                    'Failed to create GSSAPI service credential name "HTTP/%s@" for Kerberos mechanism: %s',
                    hostname,
                    exc,
                )
            else:
                self.logger.info(
                    'KerberosAuthMiddleware is identifying as "%s"', self.service
                )
